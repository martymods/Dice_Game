...
function setupSinglePlayer() {
        ...
        if (currentBet > 0) {
            startDiceRollAnimation(() => {
                ...
                if (rollsLeft === 1) {
                    ...
                    if (balance >= rent) {
                        balance -= rent;
                        rent *= 2;  // Double rent each progression
                        rollsLeft = maxTurns;
                        monthCounter++;
                        rentStatus.textContent = `Rent Due: $${rent.toLocaleString()} in ${rollsLeft} rolls`;
                        monthCounterElem.textContent = `Month: ${monthCounter}`;
                ...
                if (turns % 3 === 0 && allowItemPurchase) {
                    showItemPopup();
                    allowItemPurchase = false;
                }
            });
        } else {
    ...
    function showItemPopup() {
        ...
        itemList.innerHTML = '';
        const itemsToShow = availableItems.slice(0, 3);  // Show 3 items
