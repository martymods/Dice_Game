    ...
    if (roomName && playerName) {
        ...
        socket.on('gameCreated', (data) => {
            ...
            if (data.success) {
                ...
                window.location.href = `/game.html?room=${data.roomName}`; // You could redirect to game page here
            }
        });
    } else {
        alert('Please provide both room name and player name.');
    }
}
...
function showJoinGame() {
    ...
    if (roomName && playerName) {
        ...
        socket.on('playerJoined', (data) => {
            ...
            // Transition to game interface
            window.location.href = `/game.html?room=${data.roomName}`; // Redirect to game page
        });
        socket.on('error', (err) => {
