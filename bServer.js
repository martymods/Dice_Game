...
app.post('/placeBetHeadsOrTails', async (req, res) => {
    const { playerAddress, betAmount } = req.body;

    if (!playerAddress || !betAmount || betAmount <= 0) {
        return res.status(400).send({ success: false, error: "Invalid bet details" });
    }

    try {
        console.log(`Heads or Tails bet placed: ${betAmount} ETH by ${playerAddress}`);

        // Emit bet event to update UI in real-time
        io.emit('headsOrTailsBetPlaced', { playerAddress, betAmount });

        res.status(200).send({ success: true, message: "Heads or Tails bet placed successfully!" });
    } catch (error) {
        console.error("Error placing Heads or Tails bet:", error);
